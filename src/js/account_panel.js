...
(function( $ ) {
    ...
    var verifyApiKey = function( event ) {
        ...
        $.fn.eve_api('account/APIKeyInfo.xml', {
            ...
            'success': function(xml) { apiKeyVerificationResault(xml, form); },
            'error': function(jqXHR, status, responseText) { apiKeyVerificationFailure(form); }
        });
    ...
    var drawAccount = function(account) {
        var accForm = $('<form data-key="' + account.key + '"></form>');
        body.append(accForm);
        ...
        for (var i in account.characters) {
            ...
            checkbox.find('input').bind('change', saveCharacterDisplayStateChange)

            fieldset.append(checkbox);
        }
    };

    var saveCharacterDisplayStateChange = function( event ) {
        var input = $(event.currentTarget);
        var key = input.parents('form')[0].dataset.key;

        var acc = Account.Manager.get(key);
        acc.characters[input.val()]['display'] = input.is(':checked');
        acc.save();
    };
